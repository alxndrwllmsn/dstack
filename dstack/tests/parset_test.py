...
def setup_Parset_unittest(parset_path):
    ...
    ParsetPath: str
        ...
        It also have to contain at lest one preconditioner-specific parameter
        e.g. ``preconditioner.Wiener.robustness``!
...
class TestParset(unittest.TestCase):
    ...
    def test_preconditioning(self):
        ...
        example_parameters_for_testing = ['PWrobustness', 'PGTisPsfSize'] #Some example parameters used to test the compatibility
        for p, param in zip(preconditioners_required,example_parameters_for_testing):
            ...
            #Test compatibility through a simple example
            assert ds.parset.check_parameter_and_Preconditioner_compatibility(param, initial_parset._preconditioner) == False, \
            'The parameter {0:s} should not be compatible with the Parset, when the preconditioning {1:s} removed!'.format(param,p)
            ...
            #Test compatibility through a simple example
            assert ds.parset.check_parameter_and_Preconditioner_compatibility(param, initial_parset._preconditioner) == True, \
            'The parameter {0:s} should be compatible with the Parset, when the preconditioning {1:s} added!'.format(param,p)

        #Check compatibility when no parset is added
        initial_parset._preconditioner = []

        for param in example_parameters_for_testing:
            assert ds.parset.check_parameter_and_Preconditioner_compatibility(param, initial_parset._preconditioner) == False, \
            'The parameter {0:s} should not be compatible with the Parset, when no preconditioner is defined!'.format(param)

        #Save the parset and test if the preconditioning-related settings are removed (quick and dirty)
        initial_parset.save_parset(output_path=_TEST_DIR, parset_name=output_parset_name)
        ...
        assert len(initial_parset._parset) > len(saved_parset._parset), \
        'Preconditioning-related setting(s) were not removed or the template parset is badly configured!'
